...
def enc_digest(filename):
    ...
    encrypted_message_digest_bytes = public_key.encrypt(
        message_digest_bytes, padding.OAEP(
    ...
    decrypted_message_digest_bytes = private_key.decrypt(
        encrypted_message_digest_bytes, padding.OAEP(
            mgf=padding.MGF1(algorithm=hashes.SHA256()),
            algorithm=hashes.SHA256(),
            label=None,
        ),
...
def sign_digest(filename):
    ...
    try:
        ...
        # TODO: Task 3-8
        public_key.verify(
            signature, file_data, padding.PSS(
                mgf=padding.MGF1(hashes.SHA256()), 
                salt_length=padding.PSS.MAX_LENGTH),
                hashes.SHA256(),
        )
        print(colored("======= SIGNATURE VERIFIED =======", "yellow"))
        print("Verfication Succeeds")
