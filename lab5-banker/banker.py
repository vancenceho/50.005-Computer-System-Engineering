...
class Banker:
    ...
    def request_resources(self, customer_index, request):
        ...
        ### BEGIN ANSWER HERE ###
        # perform deepcopy of available, need, and allocation
        work = deepcopy(self.available)
        need = deepcopy(self.need)
        allocation = deepcopy(self.allocation)
        # call check_safe method with new data
        safe = self.check_safe(customer_index, request, work, need, allocation)  # Change this line
    ...
    def check_safe(self, customer_index, request, work, need, allocation):
        ...
        # hypothetically grant the current request for all i < M
        ...
        # create finish list[int] of length self.N
        finish = [False] * self.N
    
        # find index i such that both finish[i] == False, need[i] <= work
        while True:
            # initialise found to False
            found = False
            # for all i < N
            for i in range(self.N):
                # if finish[i] == False and need[i] <= work is True for all j < M
                if (not finish[i]) and all(need[i][j] <= work[j] for j in range(self.M)):
                    # for all j < M
                    for j in range(self.M):
                        # update work[j] += allocation[i][j]
        ...
        # if finish[i] == True for all i, set safe = True
        safe = all(finish)
        ...
        bank_lock.release()
        # return safe
        return safe # Change this line according to whether the request will be safe or not
